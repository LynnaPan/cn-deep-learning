...
def get_batches(int_text, batch_size, seq_length):
    ...
    dropped_text = np.reshape(dropped_text, (-1, seq_length))
    
    result_arr = np.empty((2, batch_size, seq_length))
    batches = []
    i=1
    for n in range(0, dropped_text.shape[0],  batch_size):
        inputs = dropped_text[n:n+batch_size, :]
        targets = np.zeros_like(inputs)
#         print(int_text[batch_size*seq_length*i])
        
#         print(np.append(inputs[:, 0][1:],int_text[batch_size*seq_length*i]))
        
        targets[:, :-1], targets[:, -1] = inputs[:, 1:], np.append(inputs[:, 0][1:],int_text[batch_size*seq_length*i])
...
def pick_word(probabilities, int_to_vocab):
    ...
    #words = word for woint_to_vocab(p)
