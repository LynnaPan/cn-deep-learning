...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    def generate_id(sentences_array, vocab_to_int_dict):
        id_text=[]
        for sentence in sentences_array:
            source_id = [vocab_to_int_dict[i] for i in sentence.split() ]
            id_text.append(source_id)
        return id_text
    
    #get source_id_text
    source_id_text = generate_id(source_text.split('\n'), source_vocab_to_int)
    
    #get targets_id_text
    sentences = target_text.split('\n')
    target_array = np.array([sentence+' <EOS>' for sentence in sentences])
    target_id_text = generate_id(target_array, target_vocab_to_int)
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    cutting_end = tf.strided_slice(target_data, [0,0],[batch_size, -1],[1,1])
    decode_input =  tf.concat([tf.fill([batch_size, 1], target_vocab_to_int['<GO>']), cutting_end],1)
    return decode_input
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    enc_output, enc_state = tf.nn.dynamic_rnn(enc_cell, rnn_inputs, dtype=tf.float32)
    return enc_state
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    outputs_train = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell,
                                                                  decoder_fn=dynamic_fn_train,
                                                                  inputs=dec_embed_input,
                                                                  sequence_length=sequence_length,
                                                                  scope=decoding_scope)[0]
    
    output = output_fn(outputs_train)
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    dynamic_fn_inference = tf.contrib.seq2seq.simple_decoder_fn_inference(output_fn, 
                                                                         ...
                                                                         maximum_length,
                                                                         vocab_size)
    outputs_inference = tf.contrib.seq2seq.dynamic_rnn_decoder(cell=dec_cell,
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    with tf.variable_scope("decoding_scope") as decoding_scope:
        output_fn = lambda x: tf.contrib.layers.linear(x, vocab_size,
                                        scope=decoding_scope)
        
        train_decoder = decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope, output_fn, keep_prob) 
        
        decoding_scope.reuse_variables()
        
        maximum_length = tf.reduce_max(sequence_length)
        start_of_sequence_id = tf.constant([target_vocab_to_int['<GO>']], dtype=tf.int32)
        end_of_sequence_id = tf.constant([target_vocab_to_int['<EOS>']], dtype=tf.int32)
        inference_decoder= decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id, maximum_length, vocab_size, decoding_scope, output_fn, keep_prob)
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    traing_decoder_output, inference_decoder_output = decoding_layer(dec_embed_input, dec_embeddings, 
                                                                     encoder_state, target_vocab_size, 
                                                                     sequence_length, rnn_size, num_layers, 
                                                                     target_vocab_to_int, keep_prob )
    
    return traing_decoder_output, inference_decoder_output
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    """
    # TODO: Implement Function
    source_id = [vocab_to_int.get(i, vocab_to_int['<UNK>']) for i in sentence.split() ]
    return source_id
